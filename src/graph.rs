    ...
    fn work(&self, run: &BuildRun, schedule: &Mutex<Schedule>, wakeup: &Condvar, summary: &Task) {
        while let Some((index, archives)) = self.claim(schedule, wakeup, summary) {
            ...
            let attempt = catch_unwind(AssertUnwindSafe(|| {
                node.build
                    .build_alone(run.options, run.ctx, &node.policy, run.progress, &archives)
            }));
            let result = attempt
...
struct Schedule {
    ...
    /// A build-wide cancellation request, written under the same mutex
    /// [`Graph::claim`] holds across its check-then-park sequence.
    cancelled: bool,
}
